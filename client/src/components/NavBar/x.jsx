        ...
        <nav className={`sticky top-0 left-0 w-[100%] z-20 shadow-2xl shadow-customBlack-500/20 bg-customBlack flex flex-col`}>
            <div className='w-full flex justify-between'>
                <div className="basis-1/3 flex justify-start">
                    <img src={logo} alt="logo" width={180} className='ml-[5%]'/>
                </div>
                <div className="max-[700px]:hidden basis-1/3 flex justify-around items-center flex-none ">
                    <Link to='/' className='p-[4%]' onClick={() => {menu.checked = false; setVisible(!visible)}}><i className="fa-solid fa-house fa-3x flex text-customWhite"></i></Link>
                    <Link to='/upcoming-events' className='p-[4%]' onClick={() => {menu.checked = false; setVisible(!visible)}}><i className="fa-solid fa-clock fa-3x text-customWhite"></i></Link>
                    <Link to='/account' className='p-[4%]' onClick={() => {menu.checked = false; setVisible(!visible)}}><i className="fa-solid fa-user fa-3x text-customWhite"></i></Link>
                    <Link to='/news' className='p-[4%]' onClick={() => {menu.checked = false; setVisible(!visible)}}><i className="fa-solid fa-envelope fa-3x text-customWhite"></i></Link>
                </div>
